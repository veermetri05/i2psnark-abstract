...
import org.klomp.snark.util.SyntheticREDQueue;
...
public class BandwidthManager implements BandwidthListener {
    ...
    public BandwidthManager(Environment env, int upLimit, int downLimit) {
        _env = env;
        _log = env.log(BandwidthManager.class);
        _up = new SyntheticREDQueue(_env, upLimit);
        _down = new SyntheticREDQueue(_env, downLimit);
