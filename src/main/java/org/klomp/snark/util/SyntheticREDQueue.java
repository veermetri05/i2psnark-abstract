 ...
 *  Actual queueing is assumed to be "downstream" of this.
 *
 ...
 *
 *  Port of {@code net.i2p.util.SyntheticREDQueue} (GPLv2+, I2P);
 *  {@code I2PAppContext} replaced by the SPI {@link Environment}.
 */
public class SyntheticREDQueue {
     ...
     *
     *  @param env the environment (clock, random, logs)
     *  @param bwBps the output rate of the queue in Bps
     */
    public SyntheticREDQueue(Environment env, int bwBps) {
        ...
        this(env, bwBps, bwBps / DEFAULT_LOW_THRESHOLD, bwBps / DEFAULT_HIGH_THRESHOLD);
    }
     ...
     */
    public SyntheticREDQueue(Environment env, int bwBps, int minThB, int maxThB) {
        _log = Logs.getLog(SyntheticREDQueue.class);
        _env = env;
        ...
        if (_acked < 0) {
            ...
            if (factor > 0) {
                ...
                if (_avgQSize > _minth) {
                    ...
                    float rand = _env.random().nextFloat();
                    if (rand < pa) {
                        if (_log.shouldWarn())
    ...
    public synchronized String toString() {
        return "SREDQ[" +
                ' ' + DataHelper.formatSize2Decimal((long) (_bKFiltered * 1000)) +
                "Bps, avg_qsize " +
                DataHelper.formatSize2((long) _avgQSize) +
                "B, limit " +
                DataHelper.formatSize2Decimal((long) _bwBps) +
                "Bps]";
    }
}
