...
class WebPeer extends Peer {
   ...
   */
  @Override
  public void runConnection(ClientContext ctx, PeerListener listener, BandwidthListener bwl, BitField ignore,
      ...
      try {
          ...
          while (true) {
              ...
              while (out.size() < tlen) {
                  ...
                  // do the fetch (DataFetcher sends Range + User-Agent)
                  int osz = out.size();
                  if (_log.shouldDebug())
                      _log.debug("Fetching piece: " + piece + " offset: " + off + " file offset: " + foff + " len: " + flen + " from " + url);
                  byte[] data = null;
                  try {
                      data = ctx.getDataFetcher().fetch(url, flen, HEADER_TIMEOUT, 0);
                  ...
                  if (data != null) {
                      ...
                      if (_log.shouldDebug())
                          _log.debug("Fetched piece: " + piece + " offset: " + off + " file offset: " + foff + " len: " + toCopy + " from " + url);
                      int sz = out.size() - osz;
                      if (sz != flen) {
                          if (_log.shouldWarn())
...
*/
  ...
  private synchronized Request getLowestOutstandingRequest(int piece) {
    ...
    // DataFetcher keeps lastRcvd fresh via its own progress tracking;
    // the upstream EepGet status listeners are not ported.
}
