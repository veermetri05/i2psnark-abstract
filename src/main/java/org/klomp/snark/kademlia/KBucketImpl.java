...
class KBucketImpl<T extends SimpleDataStructure> implements KBucket<T> {
     ...
     *  from us in the range [begin, end] inclusive.
     */
    public KBucketImpl(Environment context, int begin, int end, int max, KBucketTrimmer<T> trimmer) {
        if (begin > end)
            throw new IllegalArgumentException(begin + " > " + end);
        _context = context;
        _entries = Collections.newSetFromMap(new ConcurrentHashMap<T, Boolean>(max + 4));
        _begin = begin;
        _end = end;
        _max = max;
        _trimmer = trimmer;
