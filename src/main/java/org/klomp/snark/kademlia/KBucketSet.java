...
import org.klomp.snark.util.LHMCache;
...
public class KBucketSet<T extends SimpleDataStructure> {
     ...
     */
    public KBucketSet(Environment context, T us, int max, int b) {
     ...
     */
    public KBucketSet(Environment context, T us, int max, int b, KBucketTrimmer<T> trimmer) {
        _us = us;
        _context = context;
        _log = context.log(KBucketSet.class);
        _trimmer = trimmer;
        if (max <= 4 || b <= 0 || b > 8)
