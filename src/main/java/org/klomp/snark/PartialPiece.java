...
class PartialPiece implements Comparable<PartialPiece> {
    ...
    public PartialPiece (Piece piece, int len, File tempDir) {
        ...
        bitfield = new BitField((len + ClientContext.PARTSIZE - 1) / ClientContext.PARTSIZE);
        ...
        try {
            if (len <= MAX_IN_MEM) {
                try {
                    ...
                    if (_max_in_mem > ClientContext.PARTSIZE)
                        _max_in_mem /= 2;
                    Log log = Logs.getLog(PartialPiece.class);
                    log.log(Log.WARN, "OOM creating new partial piece");
    ...
    public synchronized Request getRequest() {
        int chunk = off / ClientContext.PARTSIZE;
        int sz = bitfield.size();
        for (int i = chunk; i < sz; i++) {
            if (!bitfield.get(i))
                return new Request(this, off, Math.min(pclen - off, ClientContext.PARTSIZE));
            if (i == sz - 1)
                off = pclen;
            else
                off += ClientContext.PARTSIZE;
        }
        return null;
    }
    ...
    public synchronized int getDownloaded() {
        ...
        int rv = sz * ClientContext.PARTSIZE;
        int rem = pclen % ClientContext.PARTSIZE;
        if (rem != 0 && bitfield.get(sz - 1))
            rv -= ClientContext.PARTSIZE - rem;
        return rv;
    }
    ...
    public void read(DataInputStream din, int offset, int len, BandwidthListener bwl) throws IOException {
        if (offset % ClientContext.PARTSIZE != 0)
            throw new IOException("Bad offset " + offset);
        int chunk = offset / ClientContext.PARTSIZE;
    ...
    public static void warn(String s) {
        Logs.getLog(PartialPiece.class).warn(s);
    }
}
