...
***********/
   ...
   */
/****
  Snark(ClientContext ctx, String torrent, String ip, int user_port,
...
/****
  public Snark(I2PAppContext ctx, Properties opts, String torrent,
    ...
    this(new ClientContext(ctx), torrent, null, -1, slistener, null, null, null, null, false, rootDir);
    ...
    _ctx.setI2CPConfig(host, port, opts);
...
****/
   ...
   */
  public Snark(ClientContext ctx, String torrent, String ip, int user_port,
        StorageListener slistener, CoordinatorListener clistener,
        CompleteListener complistener, PeerCoordinatorSet peerCoordinatorSet,
        ConnectionAcceptor connectionAcceptor, String rootDir)
  {
      this(ctx, torrent, ip, user_port, slistener, clistener, complistener,
    ...
    try
      {
        ...
        if (f.exists())
         ...
         /**** No, we don't ever fetch a torrent file this way
            ...
            File torrentFile = _ctx.get(torrent, 3);
            if (torrentFile == null) {
        ...
        try
          {
            ...
            if (baseFile == null) {
                ...
                if (_ctx.getFilesPublic())
                    baseFile = new File(rootDataDir, base);
                else
                    baseFile = new File(rootDataDir, base);
            }
            storage = new Storage(_ctx, baseFile, meta, slistener, shouldPreserve);
...
*/
   ...
   */
  protected Snark(ClientContext ctx, String torrent, byte[] ih, String trackerURL,
        CompleteListener complistener, PeerCoordinatorSet peerCoordinatorSet,
   ...
   *  @since 0.8.4
   */
  public Snark(ClientContext ctx, String torrent, byte[] ih, String trackerURL,
        CompleteListener complistener, PeerCoordinatorSet peerCoordinatorSet,
        ConnectionAcceptor connectionAcceptor, String rootDir)
  {
    completeListener = complistener;
  ...
  private void x_startTorrent() {
    ...
    if (!ok)
        fatalRouter(_ctx.getString("Error connecting to I2P - check your I2CP settings!"), null);
    if (coordinator == null) {
        if (_ctx.getStreamServer() == null)
            fatalRouter("Unable to listen for I2P connections", null);
        else {
            org.klomp.snark.spi.PeerIdentity d = _ctx.getMyDestination();
            if (d != null && _log.shouldLog(Log.INFO))
        ...
        activity = "Collecting pieces";
        coordinator = new PeerCoordinator(_ctx, id, infoHash, meta, storage, this, this, completeListener.getBandwidthListener());
        ...
        if (_peerCoordinatorSet != null) {
            ...
            // single torrent
            acceptor = new ConnectionAcceptor(_ctx, new PeerAcceptor(coordinator));
        }
        // TODO pass saved closest DHT nodes to the tracker? or direct to the coordinator?
        trackerclient = new TrackerClient(_ctx, meta, additionalTrackerURL, coordinator, this);
    }
...
/****
                              ...
                              CoordinatorListener clistener)
  {
    ...
    ClientContext util = new ClientContext(I2PAppContext.getGlobalContext());
...
****/
  ...
  public void gotMetaInfo(PeerCoordinator coordinator, MetaInfo metainfo) {
      try {
          ...
          storage = new Storage(_ctx, baseFile, metainfo, this, false);
          storage.check();
  ...
  public boolean overUploadLimit(int uploaders) {
    ...
    int limit = _ctx.getMaxUploaders();
