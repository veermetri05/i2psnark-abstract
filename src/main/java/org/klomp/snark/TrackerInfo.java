...
class TrackerInfo
{
  ...
  public TrackerInfo(InputStream in, byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx)
    throws IOException
  {
    this(new BDecoder(in), my_id, infohash, metainfo, ctx);
  }

  private TrackerInfo(BDecoder be, byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx)
    throws IOException
  {
    this(be.bdecodeMap().getMap(), my_id, infohash, metainfo, ctx);
  }

  private TrackerInfo(Map<String, BEValue> m, byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx)
    throws IOException
  {
        ...
        if (bePeers == null) {
          peers = Collections.emptySet();
            ...
            try {
              ...
              p = getPeers(bePeers.getBytes(), my_id, infohash, metainfo, ctx);
            } catch (InvalidBEncodingException ibe) {
              // List of Dictionaries or List of Strings
              p = getPeers(bePeers.getList(), my_id, infohash, metainfo, ctx);
            }
            peers = p;
        }
...
******/
  ...
  public TrackerInfo(Set<Hash> hashes, int interval, int complete, int incomplete, String error,
                     byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx) {
      peers = getPeers(hashes, my_id, infohash, metainfo, ctx);
  ...
  /** List of Dictionaries or List of Strings */
  private static Set<Peer> getPeers(List<BEValue> l, byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx)
    throws IOException
  {
   ...
   *  @since 0.8.1
   */
  private static Set<Peer> getPeers(byte[] l, byte[] my_id, byte[] infohash, MetaInfo metainfo, ClientContext ctx)
    throws IOException
  {
  ...
  private static Set<Peer> getPeers(Set<Hash> hashes, byte[] my_id,
                                    byte[] infohash, MetaInfo metainfo, ClientContext ctx) {
    if (hashes == null)
