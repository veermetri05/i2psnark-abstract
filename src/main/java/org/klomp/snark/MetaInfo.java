...
{  
  private final Log _log = Logs.getLog(MetaInfo.class);
           ...
           List<List<String>> announce_list, String created_by, List<String> url_list, String comment)
  {
    ...
    this.creation_date = Clock.getInstance().now();
                  ...
                  String new_created_by, List<String> new_url_list)
  {
    ...
    this.creation_date = old.creation_date > 0 ? old.creation_date : Clock.getInstance().now();
...
****/
  ...
  {
    return "MetaInfo[info_hash='" + DataHelper.toString(info_hash)
      + "', announce='" + announce
      + "', name='" + name
      + "', files=" + files
  ...
  private byte[] calculateInfoHash()
  {
    ...
    //_log.debug("info bencoded: [" + Base64.encode(infoBytes, true) + "]");
        ...
        if (_log.shouldLog(Log.DEBUG))
            _log.debug("info hash: " + DataHelper.toString(hash));
        return hash;
  }

  /**
   *  A SHA-1 message digest — pure replacement for I2P's SHA1 util.
   */
