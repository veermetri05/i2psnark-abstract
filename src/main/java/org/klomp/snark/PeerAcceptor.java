...
class PeerAcceptor
{
  private final Log _log = Logs.getLog(PeerAcceptor.class);
  ...
  public void connection(Stream socket,
    ...
    throws IOException
  {
    ...
    if (in instanceof BufferedInputStream) {
        ...
        socket.setReadTimeout((int) HASH_READ_TIMEOUT);
        try {
        ...
        try {
          ...
          if (_log.shouldLog(Log.INFO))
              _log.info("infohash read from " + socket.getPeer().calculateHash().toBase64() 
                        + ": " + Base64.encode(peerInfoHash));
        } catch (IOException ioe) {
            if (_log.shouldLog(Log.INFO))
                _log.info("Unable to read the infohash from " + socket.getPeer().calculateHash().toBase64());
            throw ioe;
        }
