...
/*
 *  A Node ID, Hash, and port, and an optional PeerIdentity.
 ...
 *  always have the PeerIdentity.
 *  The conpact info is immutable. The PeerIdentity may be added later.
 *
 * @since 0.9.2
 * @author zzz
 */
...
public class NodeInfo extends SimpleDataStructure {
     ...
     */
    public NodeInfo(PeerIdentity dest, int port) {
        super();
     ...
     */
    public NodeInfo(NID nID, PeerIdentity dest, int port) {
        super();
        this.nID = nID;
        this.dest = dest;
        this.hash = dest.calculateHash();
     ...
     * @throws IllegalArgumentException
     */
    public NodeInfo(String s, PeerIdentityFactory factory) throws DataFormatException {
        super();
        ...
        hash = Hash.create(h);
        if (parts[2].length() > 0)
            dest = factory.fromBase64(parts[2]);
        try {
    ...
    public PeerIdentity getDestination() {
        return this.dest;
    }
     ...
     */
    public void setDestination(PeerIdentity dest) throws IllegalArgumentException {
        if (this.dest != null)
            return;
